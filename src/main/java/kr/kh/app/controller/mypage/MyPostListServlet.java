...
public class MyPostListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int totalCount = boardService.getLikePostCount(cri);
		PageMaker pm = new PageMaker(5, cri, totalCount);
		request.setAttribute("pm", pm);
		//현재 페이지 정보에 맞는 게시글 리스트를 가져옴
		ArrayList<PostVO> myList = boardService.getLikePostList(cri);
		request.setAttribute("myList", myList);//화면에 전송
		request.getRequestDispatcher("/WEB-INF/views/mypage/myLikeList.jsp").forward(request, response);
	}
}
